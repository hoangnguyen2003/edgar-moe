...
export function GovernancePage() {
  ...
  if (governance.error) {
    return <div className="page"><ErrorState error={governance.error} /></div>;
  }
...
function FrozenIdentity({ identity }: { identity: GovernanceResponse["frozen_v1"] }) {
  return (
    <article className="panel governance-identity-panel">
      ...
      <p className="panel__note"><LockKeyhole size={14} /> Research-only output; changing this identity is a new reviewed v1.</p>
    </article>
  );
}
...
function BoundaryRow({ label, value, safe }: { label: string; value: string; safe: boolean }) {
  return (
    <div className="governance-boundary-row">
      {safe ? <CheckCircle2 size={16} /> : <Clock3 size={16} />}
      <span>{label}</span>
...
function ControlRow({ control }: { control: GovernanceControl }) {
  ...
  return (
    <div className={`governance-control ${enforced ? "" : "governance-control--pending"}`}>
      {enforced ? <CheckCircle2 size={16} /> : <Clock3 size={16} />}
      <div><strong>{control.key.replaceAll("_", " ")}</strong><span>{control.summary}</span></div>
