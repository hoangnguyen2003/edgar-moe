...
def _write_catalog(root: Path, *, count: int) -> tuple[Path, Path, Path, Path]:
    adr_dir = root / "adr"
    adr_dir.mkdir()
    ...
    return adr_dir, index, readme, architecture


def test_adr_catalog_matches_all_references(tmp_path: Path) -> None:
    adr_dir, index, readme, architecture = _write_catalog(tmp_path, count=2)
