...
| P0 | Partial-write reconciliation | The artifact layer now has a regression test showing that a mirror outage leaves the content-addressed primary bytes retryable without changing their identity. Still required: run the Go [evidence auditor](evidence-auditor.md) after an injected production-like object-store failure, demonstrate failed-run visibility and missing evidence, and record a safe retry/reconciliation without changing original forecasts. |
| P1 | Alert delivery | Failed forward cycles now retain a redacted context artifact and run summary. Still required: distinguish unavailable API, stale runner, failed runs, and quality warnings; trigger a controlled failure; and prove an alert reaches an agreed recipient. UI status and retained artifacts are not notification. |
