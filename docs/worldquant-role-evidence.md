...
| --- | --- | --- | --- |
| [Backend Engineer](https://www.worldquant.com/career-listing/?id=4665338006): Python APIs, database, CI, reliability and performance | Read-only [FastAPI](../src/edgar_moe/api/app.py), [registry](../src/edgar_moe/forward/registry.py), [CI](../.github/workflows/ci.yml), and [synthetic read-path benchmark](../reports/forward_read_path_benchmark.json) | No hosted API load/latency distribution under a stated traffic model | Collect a separate read-only hosted latency/error baseline and failure-injection evidence; local sequential SQLite timings are not an SLO |
| [Full Stack Engineer](https://www.worldquant.com/career-listing/?id=4658486006): REST, React, responsive and accessible UI, deployment | [Research page](../apps/web/src/pages/ResearchPage.tsx), [read-only evidence API](../src/edgar_moe/api/research_evidence.py), [page tests](../apps/web/src/pages/ResearchPage.test.tsx) now surface v1's retrospective universe screen beside its interval and pending v2 | No reviewed v2 empirical result exists | Publish an approved aggregate-only v2 report after separate study and review; test the live deployment contract |
...
| [Quantitative Researcher](https://www.worldquant.com/career-listing/?id=4069478006): rigorous signal research and C/C++ on Unix | [Locked negative result](../reports/authenticated_research_report.md), [selection](../src/edgar_moe/modeling/walk_forward.py), [leakage tests](../tests/integration/test_authenticated_study.py), and explicit [universe-bias disclosure](../src/edgar_moe/api/research_evidence.py) | v1's XBRL, baseline, and retrospective-universe defects need a separately identified, historically valid v2 evaluation; C++ proficiency is unproved | [v2 pretest review](../src/edgar_moe/modeling/pretest_review.py) plus historical-membership gate [#292](https://github.com/hoangnguyen2003/edgar-moe/issues/292); degree/class-rank requirements are not project evidence |
| [Senior Backend Engineer](https://www.worldquant.com/career-listing/?id=4653484006): database design, query optimization, API/network boundaries | [Migrations](../migrations/versions/20260921_0002_append_only_triggers.py), [reader-role audit](../src/edgar_moe/forward/reader_role.py), [API tests](../tests/integration/test_forward_api.py), and [profiled cold-interval optimization](adr/0030-forward-interval-read-path.md) | No hosted Postgres query-plan or p95 evidence under realistic traffic | Collect safe Postgres `EXPLAIN (ANALYZE, BUFFERS)` and hosted read-only latency/error baseline; senior tenure/mentoring are not provable here |
...
| [Software Engineer (C/C++)](https://job-boards.greenhouse.io/worldquant/jobs/4699962006): modern C++, memory/I/O, distributed time-series storage | [Go evidence auditor](../tools/evidence-auditor/audit.go) shows an independent storage-integrity boundary; [capacity baseline](../src/edgar_moe/capacity.py) and [ADR 0030](adr/0030-forward-interval-read-path.md) document a Python optimization before native code | No C++ implementation or profiled native bottleneck after the simpler optimization; no evidence of distributed-storage experience | Scoped native microbenchmark and parity tests **only if** later profiling meets the native-code decision gate; do not add résumé-only C++ |
