...
- **Live system:** [edgar-moe.vercel.app](https://edgar-moe.vercel.app) and its [API reference](https://edgar-moe.vercel.app/api/docs).
- **Decision log:** [21 architecture decision records](adr/README.md).
...
| Security | The public serving tier is compromised. It cannot write evidence, because routes are GET-only and sessions read-only. | Enforced in code ([ADR 0011](adr/0011-api-read-only-statement-boundary.md)). **Pending:** a verified provider reader role. |
...
| Timeliness | A scheduled run starts late. Its margin before the open is recorded, with a warning below 90 minutes. | Enforced ([ADR 0020](adr/0020-pre-open-schedule-margin.md)). Punctual scheduling is not guaranteed. |
...
| The serving tier is compromised | GET-only routes, read-only sessions, a statement timeout, and separate reader and writer URLs. | The provider reader grant is not yet verified; see [§9](#9-risks-and-open-items). |
...
| Scheduler lateness ([#156](https://github.com/hoangnguyen2003/edgar-moe/issues/156)) | Runs finish roughly 40–100 minutes before the open | Configure the `scheduler` environment, deploy and observe the optional [Cloudflare scheduler adapter](../ops/scheduler/README.md) with its manual workflow, then cut over in a separate PR; an earlier cron remains a research-design change | Maintainer |
