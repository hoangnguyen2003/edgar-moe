...
- **Decision log:** [23 architecture decision records](adr/README.md).
...
| Timeliness | A scheduled run starts late. Its margin before the open is recorded, with a warning below 90 minutes. | Enforced ([ADR 0020](adr/0020-pre-open-schedule-margin.md)); a manual read-only observer verifies the selected dispatch event, workflow path, ref, commit, and recency. Punctuality and scheduler origin are not guaranteed by GitHub metadata. |
