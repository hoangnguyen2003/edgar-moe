...
| Research copilot | Operator-run LLM explanation, evidence navigation, and human quality review | Bounded read-only tools; saved answer envelopes are independently verified before review, while citations, frozen identity, structural evaluation, and append-only rubric history are retained; no forecast, registry, artifact, or GitHub writes |
