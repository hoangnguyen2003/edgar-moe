...
| Research copilot | Operator-run LLM explanation, evidence navigation, diagnostic-history comparison, human quality review, and bounded usage observability | Bounded read-only tools; answer envelopes are verified at generation, CLI write, benchmark, and evaluation boundaries before review, while citations, frozen identity, content-addressed policy/tool-contract identity, structural evaluation, append-only rubric history, hash-pinned readiness, and non-sensitive request/latency/token telemetry are retained; diagnostic history is explicitly supplied and independently verified; no forecast, registry, artifact, or GitHub writes |
