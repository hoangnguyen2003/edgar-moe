...
| Research copilot | Operator-run LLM explanation, evidence navigation, and human quality review | Bounded read-only tools; answer envelopes are verified at generation, CLI write, benchmark, and evaluation boundaries before review, while citations, frozen identity, structural evaluation, and append-only rubric history are retained; no forecast, registry, artifact, or GitHub writes |
